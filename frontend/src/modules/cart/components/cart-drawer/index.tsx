...
import { isOptimistic } from "@lib/util/optimistic-cart"
...
export default function CartDrawer({ cart }: CartDrawerProps) {
  ...
  const { isOpen, close, setEnabled } = useCartDrawer()

  // Register with the context so ATC / AppBar know the drawer is live
  // (admin cart_drawer_enabled). Unregister on unmount.
  useEffect(() => {
    setEnabled(true)
                      ...
                      .map((item) => (
                        <div
                          ...
                          className={`flex gap-4 group transition-opacity ${
                            isOptimistic(item) ? "opacity-60" : "opacity-100"
                          }`}
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0 flex flex-col justify-between">
                            ...
                            <div className="flex items-center justify-between mt-1.5">
                              ...
                              {/* DeleteButton renders its own trash icon —
                                  pass text only, or it shows two. */}
                              <DeleteButton
                                id={item.id}
