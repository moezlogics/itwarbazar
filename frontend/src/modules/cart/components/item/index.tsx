...
import { setOptimisticQuantity } from "@lib/util/optimistic-cart"
...
export default function Item({ item, type = "full", currencyCode }: ItemProps) {
  ...
  const changeQuantity = async (quantity: number) => {
    setError(null)
    setUpdating(true)
    // Reflect the new count in the badge / drawer instantly.
    const rollback = optimisticCartUpdate((c) =>
      setOptimisticQuantity(c, item.id, quantity)
    )
    try {
      const fresh = await updateLineItem({ lineId: item.id, quantity })
      applyCart(fresh)
      router.refresh()
    } catch (err: any) {
      rollback()
