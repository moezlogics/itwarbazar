...
import ProductReviews from "@modules/products/components/product-reviews/lazy"
...
import AppBar from "@modules/common/components/app-bar"
...
const ProductTemplate = async ({
  ...
  // Size-chart image. Admin site-setting wins so an operator can swap it
  // without a deploy; the env var is the per-store default (this codebase
  // is deployed across several stores, so the URL is never hardcoded).
  // Unset → the PDP simply doesn't render the link.
  const sizeChartUrl: string | null =
    ((settings as any)?.size_chart_url as string)?.trim() ||
    process.env.NEXT_PUBLIC_SIZE_CHART_URL ||
    null
  ...
  // measured ~42KB of duplicate JSON on every PDP response.
  ...
  return (
    <>
      ...
      {/* Mobile app bar — back / "Product Details" / cart. Its presence
          also hides the main header + announcement bar + bottom tab bar
          on phones via the body:has(#pdp-app-bar) rule in globals.css,
          so the PDP reads like a native app screen (FootFlare style). */}
      {/* Belt-and-braces for browsers without :has(): this inline script
          executes the moment the streamed HTML is parsed (long before
          hydration) and flips the same body class the legacy CSS rules
          key off — so the storefront chrome never flashes. React does
          NOT re-execute dangerouslySetInnerHTML scripts on client-side
          navigations; there ProductActions' effect applies the class. */}
      <script
        dangerouslySetInnerHTML={{
          __html: "document.body.classList.add('is-pdp-page')",
        }}
      />
      <AppBar title="Product Details" />

      {/* Top breadcrumb — desktop/tablet only; the mobile app bar
          replaces it (apps navigate back, they don't show crumbs). */}
      <div className="hidden small:block container-anvogue pt-2 md:pt-4">
      ...
      >
        {/* Mobile Layout (lg:hidden)
            ALIGNMENT: every child sits flush to the container's own 16px
            gutter — no extra `px-*` here. The blocks below used to add
            `px-4` on top of `.container-anvogue`'s 1rem, so the gallery
            was inset 16px while the title/actions were inset 32px, which
            is what made the page look misaligned.
            `gap-4.5` was also a no-op: Tailwind's default spacing scale
            has no 4.5 step, so that class compiled to nothing and the
            sections had NO vertical rhythm at all. */}
        <div className="lg:hidden flex flex-col gap-4">
          {/* Main Product Image/Video Gallery */}
          ...
          <div className="flex flex-col gap-4 pb-6">
            ...
            <ProductActions
              ...
              region={region}
              whatsappNumber={(settings as any).whatsapp_number || undefined}
              whatsappBuyNowEnabled={(settings as any).whatsapp_buy_now_enabled !== "false"}
      ...
      <div id="reviews" className="container-anvogue my-6 md:my-10 scroll-mt-20">
        <ProductDescriptionTabs
          ...
          /* Pass the product's REAL specs only.
             We used to merge in `price_rs: <cheapest price>` here. That
             meant every product — even one with no specs at all — ended
             up with a non-empty spec object, so the "Specifications" tab
             always rendered AND won the default-tab race (the tab picker
             prefers specs), showing a lone "Price" row. The price is
             already displayed prominently above; it isn't a spec. */
          specs={(product.metadata as any)?.specs}
