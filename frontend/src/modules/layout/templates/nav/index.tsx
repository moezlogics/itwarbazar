...
export default async function Nav() {
  // IMPORTANT: no getLocale() here. It reads the locale COOKIE, and any
  // cookies() access inside this shared Nav made every ISR page throw
  // "Page changed from static to dynamic at runtime" (500) on its first
  // uncached render — homepage AND product pages. The language selector
  // reads the (non-httpOnly) locale cookie client-side instead.
  ...
  const Logo = ({ size = "md", isMobile = false }: { size?: "sm" | "md" | "lg"; isMobile?: boolean }) => {
    ...
    const h =
      size === "sm" ? "h-8" : size === "lg" ? "h-9 md:h-10" : "h-7 md:h-8"
    return logoUrl ? (
  ...
  return (
    <header
      id="header"
      /* z-50 (NOT 40) on purpose: `sticky` + z-index makes this header its
         own stacking context, so the slide-out side menu rendered INSIDE it
         can never paint above a sibling with an equal-or-higher z-index no
         matter what z-index the menu itself uses. The mobile bottom tab bar
         is fixed at z-40 and comes later in the DOM, so at an equal z-40 the
         tab bar won.  Result: the open menu was cut off behind the footer
         bar. Header and bottom bar never overlap visually (one is pinned to
         the top, the other to the bottom), so lifting the header one level
         is safe and fixes the menu overlay. */
    ...
    >
      ...
      {/* === Mobile header — 56px main row, NO category bar === */}
      <div className="small:hidden bg-header border-b border-header-line shadow-sm">
        <div className="grid grid-cols-[40px_1fr_40px] items-center gap-1 px-2 h-14">
          ...
          <LocalizedClientLink
            href="/"
            aria-label={siteName}
            className="flex items-center justify-center min-w-0 text-header-fg"
            data-testid="nav-store-link-mobile"
          >
            <Logo size="sm" isMobile />
          </LocalizedClientLink>

          <div className="w-10 h-10 flex items-center justify-center justify-self-end shrink-0">
            <Suspense
              fallback={
                <LocalizedClientLink
                  href="/cart"
                  aria-label="Cart"
                  className="relative w-10 h-10 flex items-center justify-center rounded-full hover:bg-header-hover text-header-fg hover:text-header-accent transition-all active:scale-90"
                >
                  <i className="ph-bold ph-handbag text-[20px]" aria-hidden />
                </LocalizedClientLink>
              }
            >
              <CartButton />
