...
const CartDropdown = ({
  ...
  const { open: openCartDrawer, enabled: cartDrawerEnabled } = useCartDrawer()
  ...
  // After ATC: prefer the slide-in cart drawer (works on mobile). Fall
  // back to the desktop hover popover when the drawer is disabled.
  useEffect(() => {
    if (itemRef.current === totalItems) return
  ...
  }, [totalItems, pathname, cartDrawerEnabled])
  ...
  return (
    ...
    >
      <Popover className="relative h-full">
        ...
        >
          <CartTrigger variant="header" count={totalItems} />
        </PopoverButton>
        <Transition
        ...
        >
          ...
          >
            ...
            {cartState && cartState.items?.length ? (
              <>
                <div className="overflow-y-auto overscroll-contain max-h-[402px] px-4 py-4 grid grid-cols-1 gap-y-5 no-scrollbar">
                  {cartState.items
                    .sort((a, b) =>
