...
const LanguageSelect = ({
  ...
  // The Nav renders server-side WITHOUT reading the locale cookie (a
  // cookies() call there broke ISR for every page). The `_medusa_locale`
  // cookie is intentionally non-httpOnly, so hydrate the selection from
  // document.cookie after mount instead.
  const [cookieLocale, setCookieLocale] = useState<string | null>(null)
  useEffect(() => {
    if (currentLocaleProp) return
    const m = document.cookie.match(/(?:^|; )_medusa_locale=([^;]+)/)
    if (m) setCookieLocale(decodeURIComponent(m[1]))
  }, [currentLocaleProp])
  const currentLocale = currentLocaleProp ?? cookieLocale
