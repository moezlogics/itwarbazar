...
const AccountLayout: React.FC<AccountLayoutProps> = ({
  ...
  return (
    <div className="bg-surface/40 min-h-screen" data-testid="account-page">
      {/* ── Mobile top bar ──────────────────────────────────
          Signed-in: the richer account bar (avatar + greeting + points).
          Guest: the standard app bar. One of the two ALWAYS renders —
          /account is in the `app-bar-route` list, so the storefront
          header is hidden here and the screen would otherwise have no
          header or back button at all. */}
      {customer ? (
        <AccountMobileTopBar
          customer={customer}
      ...
      <div className="container-anvogue small:py-10">
        <div className="grid grid-cols-1 small:grid-cols-[240px_1fr] small:gap-8">
          ...
          <main className="min-w-0">
            ...
            <div className="py-4 small:py-0">{children}</div>
          </main>
        </div>
      </div>
    </div>
