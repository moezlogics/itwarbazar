...
export async function middleware(request: NextRequest) {
  ...
  // Prevent indexation of filtered or sorted URLs. Facet combinations are
  // effectively infinite, so every one of them must stay out of the index
  // while the clean archive URL — which is what `alternates.canonical`
  // points at — remains the single indexable version of the page.
  //
  // Prefixes match lib/util/facets: cat_* (category facets), opt_*
  // (variant options), spec_* (metadata specs), plus brand and the
  // price / stock / sort / paging params.
  const hasFilterParams = Array.from(request.nextUrl.searchParams.keys()).some(
    (key) =>
      key.startsWith("spec_") ||
      key.startsWith("cat_") ||
      key.startsWith("opt_") ||
      ["brand", "minPrice", "maxPrice", "inStock", "sortBy", "page"].includes(key)
  )
  if (hasFilterParams) {
