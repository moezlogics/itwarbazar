...
export default async function CheckoutLayout({
  ...
  return (
    <div className="w-full bg-bg relative md:min-h-screen">
      {/* Mobile: the same app bar the product/cart screens use, so the
          whole "sub-screen" flow feels consistent. Desktop keeps the
          branded checkout header below. */}
      <AppBar title="Checkout" showCart={false} fallbackHref="/cart" />

      <header className="hidden small:block h-14 bg-header border-b border-header-line text-header-fg">
