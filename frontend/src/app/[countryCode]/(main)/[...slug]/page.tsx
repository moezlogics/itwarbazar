...
// ISR (revalidate + empty generateStaticParams): each product / brand /
// category URL renders once on first hit, then serves from the full-route
// cache and refreshes in the background. For this to keep working, the
// WHOLE render tree must stay free of dynamic APIs: no cookies()
// (auth/cart/locale hydrate client-side) and no `await searchParams`
// (filters/sort/pagination are client-side). One violation = every
// uncached hit 500s with DYNAMIC_SERVER_USAGE.
...
export async function generateMetadata(props: Props): Promise<Metadata> {
  ...
  // IMPORTANT: never `await props.searchParams` here (or in the page).
  // Reading the query string forces dynamic rendering, which makes this
  // ISR route throw DYNAMIC_SERVER_USAGE (500) on every uncached hit.
  // Filtered-URL noindex is handled by the middleware's X-Robots-Tag
  // header instead — filters/sort/pagination are all client-side.
...
export default async function SlugPage(props: Props) {
  // No `await props.searchParams` — see the note in generateMetadata.
  // Sorting / pagination / filters all resolve client-side.
  const params = await props.params
  const { slug: segments, countryCode } = params
  ...
  if (pricedProduct) {
    ...
    if (
      ...
      permanentRedirect(`/${countryCode}${canonicalPath}`)
    }
    ...
    return (
      <>
        ...
        <StoreTemplate
          ...
          countryCode={countryCode}
          /* Banners targeted at THIS brand page. */
          bannerPlacement={`brand:${brand.id}`}
        >
