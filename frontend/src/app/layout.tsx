...
export default async function RootLayout(props: { children: React.ReactNode }) {
  ...
  return (
    ...
    >
      <head>
        {/* ── App-mode routes: no storefront header, no flash ─────────────
            The layout streams <Nav/> BEFORE the page body, so a CSS rule
            keyed off a page's own markup (`body:has(#app-bar)`) can only
            hide the header AFTER it has already painted — the visible
            "full header, then the app bar" flash.

            These routes all have fixed, predictable URLs, so we decide
            here in <head>, before the header element is even parsed. Keep
            this list in sync with `APP_BAR_ROUTE_RE` in client-wrappers
            (which re-applies it on client-side navigation). */}
        <script
          dangerouslySetInnerHTML={{
            __html:
              "(function(){try{if(/^\\/(?:[a-z]{2}\\/)?(?:products|cart|checkout|account)(?:\\/|$)/.test(location.pathname)){document.documentElement.classList.add('app-bar-route')}}catch(e){}})();",
          }}
        />
        ...
        {/* Product images come from the self-hosted CDN — open the
            DNS+TLS connection during HTML parse so the LCP hero/product
            image request doesn't pay the handshake on slow mobile. */}
        ...
        {/* LocalBusiness JSON-LD — emits ClothingStore / GroceryStore / Pharmacy / Store
