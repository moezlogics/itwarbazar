// NOTE: deliberately NO @lib/config (Medusa SDK) import here — this file
// is pulled into the CLIENT bundle by <ProductReviews>, and the SDK drags
// ~28 kB gz (SDK + Buffer polyfill + qs) onto every product page.
import { storeFetch, STORE_BACKEND_URL } from "./store-fetch"

const BACKEND_URL = STORE_BACKEND_URL
...
export async function fetchProductReviews(
  ...
  try {
    const data = await storeFetch<{ reviews: ProductReview[] }>(
      "/store/reviews",
      {
...
export async function fetchProductReviewStats(
  ...
  try {
    const data = await storeFetch<{ count: number, averageRating: number }>(
      "/store/reviews",
      {
...
export async function submitProductReview(
  ...
  return storeFetch("/store/reviews", {
    method: "POST",
