...
export async function retrieveCart(cartId?: string, fields?: string) {
  ...
  fields ??=
    "*items, *region, *items.product, *items.product.metadata, *items.variant, *items.thumbnail, *items.metadata, +items.total, *promotions, *shipping_methods, *payment_collection, *payment_collection.payment_sessions"
  ...
  return await sdk.client
    .fetch<HttpTypes.StoreCartResponse>(`/store/carts/${id}`, {
      ...
      headers,
      // Cart is per-user and mutates often (ATC, qty, etc.). force-cache
      // served a stale empty cart after add-to-cart — badges stayed at 0
      // until a hard refresh. Always fetch fresh.
      cache: "no-store",
    })
...
export async function addToCart({
  ...
  // Return the fresh, fully-populated cart so the client can update its
  // context in ONE round-trip. Previously the caller fired a separate
  // re-hydrate (customer + cart-id + cart + shipping-options = ~4 more
  // sequential requests) which is why the drawer opened empty and only
  // filled a beat later. Now: one write, one read, done.
...
export async function addBundleToCart({
  ...
  const backendUrl =
    process.env.MEDUSA_BACKEND_URL || "http://localhost:3212"
...
export async function updateLineItem({
  ...
  // Fresh cart back to the caller — see addToCart for why.
...
export async function deleteLineItem(lineId: string) {
  ...
  // Fresh cart back to the caller — see addToCart for why. Null when the
  // last line was removed and the cart is now empty.
  return await retrieveCart(cartId)
}
...
export async function setAddressesAndPlace(currentState: unknown, formData: FormData) {
  ...
  try {
    ...
    const firstName = str("shipping_address.first_name")
    // Single-word names used to send last_name="" and fail validation
    const lastName = str("shipping_address.last_name") || firstName || "-"
    ...
    const shippingAddress = {
      ...
      address_2: "",
      company: "",
      // PK checkouts often omit postal — empty nulls break complete-cart
      postal_code: str("shipping_address.postal_code") || "00000",
    ...
    if (sameAsBilling === "on") {
      ...
      const bFirst = str("billing_address.first_name")
      data.billing_address = {
        first_name: bFirst,
        last_name: str("billing_address.last_name") || bFirst || "-",
        address_1: str("billing_address.address_1"),
        address_2: "",
        company: "",
        postal_code: str("billing_address.postal_code") || "00000",
    ...
    const mapLat = str("map_lat")
    const mapLng = str("map_lng")
    const mapAddress = str("map_address")
    const mapSource = str("map_source")
    const pushEndpoint = str("push_endpoint")
    const guestId = str("guest_id")

    // MERGE with existing cart metadata — replacing it wiped loyalty
    // redemption fields and made complete-cart fail validation.
    const existingCart = await retrieveCart(undefined, "id,+metadata")
    const metadata: Record<string, any> = {
      ...((existingCart?.metadata as Record<string, any>) || {}),
    }
    if (mapLat && mapLng) {
      metadata.map_lat = parseFloat(mapLat)
      metadata.map_lng = parseFloat(mapLng)
    ...
    if (pushEndpoint) metadata.push_endpoint = pushEndpoint
    if (guestId) metadata.guest_id = guestId
    data.metadata = metadata
...
export async function placeOrder(cartId?: string) {
  ...
  // Medusa returns type:"cart" with an error when complete fails —
  // previously this was silently ignored and the form looked stuck.
  const failMsg =
    (cartRes as any)?.error?.message ||
    (cartRes as any)?.error ||
    "Could not complete your order. Please check shipping, payment, and try again."
  throw new Error(
    typeof failMsg === "string" ? failMsg : "Could not complete your order."
  )
}
