...
export const listProducts = async ({
  ...
  try {
    ...
    try {
      const productBrandMap = await getProductBrandMap(
        products.map((p) => p.id)
      )
      for (const product of products) {
        const brand = productBrandMap[product.id]
        if (brand) {
          product.metadata = {
            ...(product.metadata || {}),
            brand: brand.handle,
      ...
      console.error("[listProducts] failed to map brands:", e)
    }
    ...
    console.error("[listProducts] fetch failed:", error)
    return {
      response: { products: [], count: 0 },
      nextPage: null,
      queryParams,
