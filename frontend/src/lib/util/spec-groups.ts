...
const HIDDEN = new Set([
  ...
  "image",
  "image_alt",
  // Pricing / Medusa price-list internals — never show on the PDP
  "price",
  "price_rs",
  "price_list",
  "price_list_id",
  "discount_price_list_id",
  "discount_price_list",
  "price_list_type",
])
...
export function buildSpecGroups(specs: any): SpecGroup[] {
  ...
  for (const [key, raw] of Object.entries(specs)) {
    ...
    if (HIDDEN.has(keyLower)) continue
    if (key.startsWith("_")) continue
    // Catch Medusa / promo internals even if keyed oddly
    if (
      keyLower.includes("price_list") ||
      keyLower.includes("discount_price_list")
    ) {
      continue
    }
