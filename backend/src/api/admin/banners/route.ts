  ...
  const [banner] = await svc.createBanners([
    {
      ...
      // Natural pixel size, measured by the uploader. Lets the storefront
      // render each banner at its own ratio with no layout shift.
      image_width: num(body.image_width),
      image_height: num(body.image_height),
    } as any,
  ])
