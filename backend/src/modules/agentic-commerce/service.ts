...
export type ChatSiteContext = {
  ...
  /** "fashion" | "electronics" | "grocery" | "pharmacy" | "general" — defaults to "fashion". */
...
`.trim(),
  fashion: `
FASHION STORE — DOMAIN GUIDANCE
- This is a Pakistani fashion, apparel, and footwear store (think J. /
  Khaadi / Sana Safinaz / Stylo / Borjan / Zeen style). Categories include
  men's wear, women's wear, kids' wear, shoes, footwear, unstitched fabric,
  ready-to-wear, and accessories.
- Local brands / expectations: Stylo, Borjan, Ndure, Bata, Servis, J. (Junaid Jamshed),
  Alkaram, Gul Ahmed, Khaadi, Sapphire, Zeen, Sana Safinaz, LimeLight, Edenrobe.
  Use these names only when they actually appear in search_products
  results — never invent stock.
- Sizing guidance: always check the product title and metadata for size details.
  For clothing (S, M, L, XL, etc. or chest sizes in inches). For shoes
  (Euro sizes like 36-45 or US/UK sizes). If the customer is unsure, check
  if there is a size chart page, or advise them to check the product details page
  specifications, and stay neutral.
- Material/Fabric details: mention the material (e.g. Lawn, Cotton, Silk, Leather,
  Synthetic, Velvet) only if the product title or metadata explicitly has it.
- For "sasta", "cheaper", "alternative", "similar" requests use
  find_substitutes — same category, sorted by price ascending.
- Care instructions: only mention wash or care guidelines if they are present
...
function buildSystemPrompt(ctx: ChatSiteContext = {}): string {
  const businessType = (ctx.businessType || "fashion").toLowerCase()
  const verticalBlock =
...
- For greetings, small talk, thanks, and general questions that do NOT need
  ...
  ask about our products (like groceries) — don't dump products on them.
...
- "national spices under 200" → search_products(query: "national", max_price: 200) or "milk under 400" → search_products(query: "milk", max_price: 400).
  Use a CATEGORY/TYPE word (like "milk", "oil", "shampoo", "biscuit") or brand — not the whole sentence.
- SPEC QUERIES — use spec_contains (reads each product's REAL saved attributes):
  "sugar free biscuits" → search_products(query: "biscuit", spec_contains: ["sugar free"]). "1kg pack dalda" → query: "dalda", spec_contains: ["1kg"].
  Then present the matches as a clear list (name + the matching attribute + price). If none match, say so honestly and suggest the closest options.
...
const TOOLS = [
  {
    ...
    function: {
      ...
      description:
        "Search the REAL store catalog. Pass a product type/brand/category word (e.g. 'milk', 'bread', 'oil', 'National', 'Nestle') — it matches product titles AND category names. Use max_price for budget queries. Use spec_contains to filter by specific attributes the customer asks for (weight, volume, pack size, sugar-free, organic, etc.) — this reads each product's real saved attributes. Returns real products with handle, title, price (PKR, major units), brand, and key_specs. ALWAYS use this before recommending — never invent products, prices, or specs.",
      parameters: {
        type: "object",
        properties: {
          query: {
            type: "string",
            description:
              "Product type / brand / category keyword (e.g. 'milk', 'National', 'Nestle'). Leave empty to list popular products.",
          },
          max_price: {
            type: "number",
            description: "Maximum price in PKR (e.g. 500 for 'under 500'). Results are sorted cheapest-first.",
          },
          min_price: {
            type: "number",
          ...
          spec_contains: {
            ...
            items: { type: "string" },
            description:
              "Attribute keywords a product MUST match in its saved specifications (matched case-insensitively against the product's specs + title; ALL terms must be present). Use for attribute-based queries. Examples: 'sugar free biscuits' → ['sugar free']; '1kg pack dalda' → ['1kg']; '500ml oil' → ['500ml']. Keep each term short (a single attribute). Combine with query (e.g. query:'biscuit') to scope the catalog first.",
          },
        },
        required: ["query"],
      },
  ...
  },
  {
    ...
    function: {
      name: "build_bundle",
      description:
        "Given a list of items the user wants as a bundle, search the catalog for each query and return a basket of best-match products the user can add to cart in one tap. Use for any 'set up X for me' request — e.g. 'breakfast bundle', 'biryani ingredients kit', 'weekly groceries', or electronics like 'gaming setup', 'WFH desk setup'.",
      parameters: {
        type: "object",
        properties: {
          items: {
  ...
  },
  {
    ...
    function: {
      ...
      description:
        "Compare 2 to 4 products side by side (price, stock, brand, key details) using their handles. Use for 'compare X vs Y', 'kaunsa behtar hai', 'difference between …'. Returns real catalog data only.",
      parameters: {
        type: "object",
        properties: {
          handles: {
