...
GEN_CONFIG="/home/itwarbazar-api/htdocs/api.itwarbazar.pk/ecosystem.config.js"
...
# One-time migration: clean up legacy server and worker processes if they exist.
if pm2 describe medusa-server >/dev/null 2>&1; then
  echo "Removing legacy 'medusa-server' process..."
  pm2 delete medusa-server || true
fi
if pm2 describe medusa-worker >/dev/null 2>&1; then
  echo "Removing legacy 'medusa-worker' process..."
  pm2 delete medusa-worker || true
fi
